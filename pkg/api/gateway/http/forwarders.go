...
func (m *messageMarshaler) Marshal(v interface{}) ([]byte, error) {
	...
	if len(m.fields) == 0 {
		dataBytes, err = m.marshaler.Marshal(v)
		if err != nil {
			return nil, err
		}
	} else {
		...
		if _, ok := v.([]interface{}); ok {
			...
			err = m.marshaler.Unmarshal(dataBytes, &data)
			if err != nil {
				return nil, err
			}
			for i := range data {
			...
			dataBytes, err = m.marshaler.Marshal(data)
			if err != nil {
				return nil, err
			}
		} else {
...
func newMarshaler(marshaler runtime.Marshaler, req *http.Request, isSSE bool) *messageMarshaler {
	...
	return &messageMarshaler{marshaler: marshaler, isSSE: isSSE, fields: fields, exclude: exclude}
}
...
func NewStreamForwarder(messageKey func(proto.Message) (string, error)) StreamForwarderFunc {
	return func(ctx context.Context, mux *runtime.ServeMux, marshaler runtime.Marshaler, w http.ResponseWriter, req *http.Request, recv func() (proto.Message, error), opts ...func(context.Context, http.ResponseWriter, proto.Message) error,
	) {
		...
		m := newMarshaler(outbound, req, isSSE)
		if messageKey != nil {
			oldRecv := recv
			recv = func() (proto.Message, error) {
				for {
