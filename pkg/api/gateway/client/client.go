...
func doHTTPRequest(ctx context.Context, req *resty.Request) (interface{}, error) {
	...
	return &httpbody.HttpBody{
		ContentType: res.Header().Get(publichttp.HeaderContentType),
		Data:        res.Body(),
	}, nil
}
...
func DoStreamingRequest[T any](ctx context.Context, req *resty.Request) (<-chan *T, <-chan error, error) {
	...
	if rawRes.IsError() {
		...
		if err := gwoption.Marshaller.Unmarshal(rawErrRes, &errRes); err != nil {
	...
	go func() {
		...
		for {
			...
			var data T
			if err := gwoption.Marshaller.Unmarshal(rawResult, &data); err != nil {
				errCh <- err
				return
			}
			resCh <- &data
